...
That bootstraps `.venv`, verifies imports and critical paths, and runs the test suite. The full training pipeline runs in Colab — see [Training Pipeline](#training-pipeline). The digital demo runs locally against the canonical patch in `outputs/yolov8n_patch_v2/patches/patch.png` — see [Live Demo](#live-demo).

Notes for reviewers:
- Model weights (`yolov8n.pt`, `yolo11n.pt`, `yolo26n.pt`) download automatically on first use via `ultralytics`.
- The slide deck (`adversarial_patch_presentation.pptx`) is a build artifact regenerated from `presentation.html` via `python scripts/html_to_pptx.py`.
